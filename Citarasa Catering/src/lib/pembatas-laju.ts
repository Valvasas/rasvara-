  ...
  try {
    ...
    if (forwarded) {
      const daftarIp = forwarded
        ...
        .map((s) => s.trim())
        .filter(Boolean);
      // Proxy menambahkan alamat yang benar-benar dilihatnya di ujung kanan.
      // Dengan `hop` proxy tepercaya, alamat klien asli ada `hop` langkah dari
      // kanan; apa pun di kirinya masih boleh dikarang klien.
