...
window.loadAdminData = async function() {
    try {
        await Promise.all([loadMenuData(), loadOrderData(), loadSettingsData(), loadSalesHistory(), loadLedgerData()]);
        renderAdminStats();
...
async function loadMenuData() {
    const response = await fetch(`${API_URL}/menus?includeCost=true`);
...
async function loadOrderData() {
    const response = await fetch(`${API_URL}/orders?includeCost=true`);
...
async function loadSalesHistory() {
    ...
    const response = await fetch(`${API_URL}/sales-history${query.toString() ? `?${query}` : ''}`);
...
async function loadLedgerData() {
    ...
    if (startDate) query.set('startDate', startDate);
    if (endDate) query.set('endDate', endDate);
    const response = await fetch(`${API_URL}/ledger${query.toString() ? `?${query}` : ''}`);
...
window.savePromoSettings = async function() {
    const payload = {
        highlight: {
            ...
            priceOld: siteSettings.highlight?.priceOld || 500000,
            priceNew: siteSettings.highlight?.priceNew || 450000,
            productId: siteSettings.highlight?.productId || adminMenus[0]?.id || 1
        }
    };
...
window.deleteLedgerEntry = async function(entryId) {
    ...
    const response = await fetch(`${API_URL}/ledger/${entryId}`, { method: 'DELETE' });
    if (!response.ok) {
        const error = await response.json();
